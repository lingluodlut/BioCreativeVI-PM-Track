...
if __name__ == '__main__':
    ...
    s = {
         ...
         'hidden_dims':30,
	 'ppi_pre':True,
	 'ppilayer_train':False,
         'mini_batch':32,
         'epochs':30,
         'model_save':False,
         'model':'CNN'} #LSTM,CNN,LSTM-CNN,RCNN

    print s


    model_file='/home/BIO/luoling/biocreativeVI/model/ppiac_premodel/bilstm-att-token-1l-50d.ppiac3_model'
    train_result_file='/home/BIO/luoling/biocreativeVI/result/NN/ppipre-token-50d-sta.result_train_post'
    dev_result_file='/home/BIO/luoling/biocreativeVI/result/NN/ppipre-token-50d-sta.result_dev_post'
    test_result_file='/home/BIO/luoling/biocreativeVI/result/NN/ppipre-token-50d-sta.result_test_post'
    folder1 = '/home/BIO/luoling/biocreativeVI/model/word_embedding_model/fastText/'
    rep1 = RepresentationLayer(folder1 + 'BioCreativevi.vec_new',POS_vocab_file='./fea_vocab/POS.vocab',NER_vocab_file='./fea_vocab/NER.vocab',vec_size=50, frequency=800000)
...
#    input_vec=word_vec
    ...
    if s['model']=='LSTM':
        ...
        dense=Dense(s['hidden_dims'],activation='relu')(pool)
        pool=Dropout(0.2)(dense)
        ...
        pool=GlobalMaxPooling1D()(cnn)
        pool=Dense(s['hidden_dims'],activation='tanh')(pool)
    ...
    if s['ppi_pre']==True:
        ppi_emb=Embedding(rep1.vec_table.shape[0], rep1.vec_table.shape[1], input_length=s['max_len'],name='emb',trainable=s['ppilayer_train'])
    ...
    predict=Dense(2,activation='softmax')(ppi_fc)
    if s['pos_fea']==True and s['ner_fea']==False:
        if s['model']=='RCNN':
    ...
    if s['pos_fea']==True and s['ner_fea']==False:
        if s['model']=='RCNN':
            train_res = model.predict([train_x1,l_train_x1,r_train_x1,train_pos],batch_size=200)
            ...
            train_res = model.predict([train_token,train_pos],batch_size=200)
            dev_res = model.predict([dev_token,dev_pos],batch_size=200)
            test_res = model.predict([test_token,test_pos],batch_size=200)

    elif s['pos_fea']==False and s['ner_fea']==True:
        if s['model']=='RCNN':
            train_res = model.predict([train_x1,l_train_x1,r_train_x1,train_ner],batch_size=200)
            ...
            train_res = model.predict([train_token,train_ner],batch_size=200)
            dev_res = model.predict([dev_token,dev_ner],batch_size=200)
            test_res = model.predict([test_token,test_ner],batch_size=200)
    elif s['pos_fea']==True and s['ner_fea']==True:
        if s['model']=='RCNN':
            train_res = model.predict([train_x1,l_train_x1,r_train_x1,train_pos,train_ner],batch_size=200)
            ...
            train_res = model.predict([train_token,train_pos,train_ner],batch_size=200)
        ...
        if s['model']=='RCNN':
            train_res = model.predict([train_x1,l_train_x1,r_train_x1],batch_size=200)
